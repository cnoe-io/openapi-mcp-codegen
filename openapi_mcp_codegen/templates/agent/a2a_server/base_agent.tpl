...
class BaseAgent(ABC):
    ...
    async def stream(
        ...
        # Track whether we're in a tool call phase
        in_tool_call = False
        ...
        async for event in self.graph.astream_events(inputs, config, version="v2"):
            ...
            debug_print(f"Event: {kind}", banner=False)

            # Handle token-level streaming from chat model
            if kind == "on_chat_model_stream":
                # Get the token chunk from the event
            ...
            # Handle tool calls
                ...
                if output and hasattr(output, "tool_calls") and len(output.tool_calls) > 0:
                    if not in_tool_call:
                        in_tool_call = True
                        yield {
                            'is_task_complete': False,
