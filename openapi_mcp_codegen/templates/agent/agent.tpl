...
# Create an isolated tracer provider to avoid external OTEL noise (e.g., A2A/uvicorn/httpx)
_lf_tracer_provider = TracerProvider()
_langfuse = Langfuse(
    tracer_provider=_lf_tracer_provider,
...
async def create_agent(prompt: str | None = None, response_format=None):
    ...
    _langfuse.update_current_trace(tags=["{{ mcp_name }}-agent"])
