...
{% if a2a_proxy %}
Note: This agent was generated with --with-a2a-proxy. It starts a minimal WebSocket JSON-RPC upstream server. You must deploy the separate a2a-proxy project, which will proxy A2A requests to this WebSocket server.
To run:
  make run-proxy
{% endif %}
...
{% else %}
    ```bash
    # Evaluation mode relies on A2A; not available when generated with --a2a-proxy
