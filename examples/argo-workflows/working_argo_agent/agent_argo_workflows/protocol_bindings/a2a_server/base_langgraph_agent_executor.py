        ...
        async for event in self.agent.stream(query, task.context_id, trace_id):
            if event['is_task_complete']:
                ...
                if kind == 'tool_result' or 'tool_result' in event:
                    ...
                    # Debug: Log the full MCP tool result
                    logger.info(f"{agent_name}: ✅ Tool result - {tool_name} ({status_text})")
                    logger.info(f"{agent_name}: 🔍 Full MCP tool result: {tool_info}")

                    # Also log the content if available
                    if 'content' in tool_info:
                        logger.info(f"{agent_name}: 📄 Tool content: {tool_info['content']}")

                    # Log error details if it's an error
                    if is_error and 'error' in tool_info:
                        logger.error(f"{agent_name}: ❌ Tool error details: {tool_info['error']}")
                    await event_queue.enqueue_event(
                        TaskArtifactUpdateEvent(
                            append=False,
                            context_id=task.context_id,
