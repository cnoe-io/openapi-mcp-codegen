...
class BaseLangGraphAgent(ABC):
    ...
    def __init__(self):
        ...
        logger.info(f"{agent_name}: ✅ LLM initialized: {self.model}")

        self.tracing = TracingManager()
        logger.info(f"{agent_name}: ✅ Tracing manager initialized")

        self.graph = None
        logger.info(f"{agent_name}: 📝 Graph initialized as None (will be set up on first request)")

        # Store tool metadata for debugging and reference
        self.tools_info = {}
        logger.info(f"{agent_name}: ✅ Agent initialization complete")
...
"""
    ...
    async def _setup_mcp_and_graph(self, config: RunnableConfig) -> None:
        ...
        if mcp_mode == "http" or mcp_mode == "streamable_http":
            ...
            logging.info(f"{agent_name}: 🔍 Custom HTTP config: {custom_http_config}")

            if custom_http_config:
                # Use custom HTTP configuration (e.g., GitHub Copilot API)
                # Use transport from custom config or default to streamable_http
                transport = custom_http_config.get("transport", "streamable_http")
                mcp_client_config = {
                    agent_name: {
                        "transport": transport,
                ...
                logging.info(f"{agent_name}: 🔍 Full MCP client config: {mcp_client_config}")
                logging.info(f"Using custom HTTP MCP configuration for {agent_name} with transport: {transport}")
        ...
        logger.info(f"{agent_name}: 🔄 Requesting tools from MCP server...")
        try:
            # Add debugging for MCP client internals
            logger.info(f"{agent_name}: 🔍 MCP client type: {type(client)}")
            logger.info(f"{agent_name}: 🔍 MCP client config: {getattr(client, '_config', 'No config attr')}")

            tools = await client.get_tools()
            logger.info(f"{agent_name}: ✅ Successfully retrieved {len(tools)} tools from MCP server")

            # Log tool names for debugging
            tool_names = [tool.name for tool in tools] if tools else []
            logger.info(f"{agent_name}: 🔍 Available tools: {tool_names}")

            # Test a simple tool call to see what happens
            if tools:
                logger.info(f"{agent_name}: 🧪 Testing InfoService_GetVersion tool directly...")
                try:
                    # Find the GetVersion tool
                    version_tool = None
                    for tool in tools:
                        if 'GetVersion' in tool.name:
                            version_tool = tool
                            break

                    if version_tool:
                        logger.info(f"{agent_name}: 🔍 Found version tool: {version_tool.name}")
                        # Try to call it directly to see what we get
                        result = await version_tool.ainvoke({})
                        logger.info(f"{agent_name}: 🔍 Direct tool call result: {result}")
                        logger.info(f"{agent_name}: 🔍 Result type: {type(result)}")
                    else:
                        logger.warning(f"{agent_name}: ⚠️ No GetVersion tool found")

                except Exception as tool_e:
                    logger.error(f"{agent_name}: ❌ Direct tool test failed: {tool_e}")
                    logger.error(f"{agent_name}: 🔍 Tool exception type: {type(tool_e).__name__}")

        except Exception as e:
            logger.error(f"{agent_name}: ❌ Failed to get tools from MCP server: {e}")
            logger.error(f"{agent_name}: 🔍 Exception type: {type(e).__name__}")
            logger.error(f"{agent_name}: 🔍 Exception details: {str(e)}")
            raise

        # Allow subclasses to wrap tools (e.g., for output chunking)
        logger.info(f"{agent_name}: 🔄 Wrapping MCP tools...")
        tools = self._wrap_mcp_tools(tools, args.get("thread_id", "default"))
        logger.info(f"{agent_name}: ✅ Tools wrapped, final count: {len(tools)}")
        ...
        logger.info(f"{agent_name}: 🔍 Memory checkpointer: {memory}")
        logger.info(f"{agent_name}: 🔍 System instruction length: {len(self._get_system_instruction_with_date())}")
        ...
        try:
            ...
            logger.info(f"{agent_name}: ✅ Graph created successfully!")
            logger.info(f"{agent_name}: 🔍 Graph type: {type(self.graph)}")

        except Exception as e:
            logger.error(f"{agent_name}: ❌ Failed to create graph: {e}")
            logger.error(f"{agent_name}: 🔍 Exception type: {type(e).__name__}")
            logger.error(f"{agent_name}: 🔍 Exception details: {str(e)}")
            raise
    ...
    async def _ensure_graph_initialized(self, config: RunnableConfig) -> None:
        ...
        agent_name = self.get_agent_name()
        logger.info(f"{agent_name}: 🔍 _ensure_graph_initialized called, graph is: {self.graph}")

        if self.graph is None:
            logger.info(f"{agent_name}: 🚀 Graph is None, initializing MCP and graph...")
            await self._setup_mcp_and_graph(config)
            logger.info(f"{agent_name}: ✅ Graph initialization complete, graph is now: {self.graph}")
        else:
            logger.info(f"{agent_name}: ✅ Graph already initialized, skipping setup")
    ...
    async def stream(
        ...
        # Ensure graph is initialized before any operations
        logger.info(f"{agent_name}: 🔄 About to ensure graph is initialized...")
        await self._ensure_graph_initialized(config)
        logger.info(f"{agent_name}: ✅ Graph initialization check complete")

        # Auto-trim old messages to prevent context overflow
        logger.info(f"{agent_name}: 🔄 About to trim messages...")
        await self._trim_messages_if_needed(config)
        logger.info(f"{agent_name}: ✅ Message trimming complete")
        ...
        if enable_streaming:
            ...
            logger.info(f"{agent_name}: 🔄 Starting graph.astream with inputs: {inputs}")
            logger.info(f"{agent_name}: 🔍 Stream mode: ['messages', 'custom']")
            processed_message_count = 0

            async for item_type, item in self.graph.astream(inputs, config, stream_mode=['messages', 'custom']):
                logger.debug(f"{agent_name}: 🔍 Received stream item - type: {item_type}, item: {item}")
                ...
                if isinstance(message, ToolMessage):
                    ...
                    logger.info(f"{agent_name}: 🔧 Processing ToolMessage - name: {tool_name}, status: {status}")
                    logger.info(f"{agent_name}: 🔍 Tool content (first 500 chars): {str(tool_content)[:500]}")
                    logger.info(f"{agent_name}: 🔍 Tool content length: {len(str(tool_content))}")
                    logger.info(f"{agent_name}: 🔍 Tool content type: {type(tool_content)}")
                    logger.info(f"{agent_name}: 🔍 Raw ToolMessage: {message}")
                    logger.info(f"{agent_name}: 🔍 ToolMessage.__dict__: {getattr(message, '__dict__', 'No __dict__')}")

                    if is_error:
                        logger.error(f"{agent_name}: ❌ Tool error detected in content: {str(tool_content)[:200]}")
                    elif not tool_content or len(str(tool_content).strip()) == 0:
                        logger.warning(f"{agent_name}: ⚠️ Tool returned empty content - this is the problem!")
                        logger.warning(f"{agent_name}: 🔍 Empty content debug - content repr: {repr(tool_content)}")
