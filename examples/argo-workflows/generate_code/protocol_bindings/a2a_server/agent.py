...
while providing argo_workflows-specific configuration and MCP server bootstrap.
"""
...
class Argo_workflowsAgent(BaseLangGraphAgent):
  """Argo_workflows agent that inherits streaming and A2A protocol handling from BaseLangGraphAgent."""

  @override
  async def _bootstrap_agent(self) -> Any:
    """Bootstrap the argo_workflows MCP server and return a LangGraph React agent."""
    load_dotenv()
  ...
  def _get_trace_tags(self) -> list[str]:
    """Return trace tags specific to argo_workflows."""
    return ["argo_workflows-a2a"]

  @override
  def _get_trace_name(self) -> str:
    """Return trace name for argo_workflows queries."""
    return "argo_workflows-query"
  ...
  def get_agent_name(self) -> str:
    ...
    return "argo_workflows-agent"

  @override
  def get_system_instruction(self) -> str:
    """Return the system instruction for the agent."""

    return """You are an expert Argo Workflows assistant that helps users manage, create, and monitor workflows in Kubernetes.
...
Do NOT include any text before or after the JSON. The "content" field contains your complete, helpful response."""
  ...
  def get_tool_working_message(self) -> str:
    ...
    return "🔄 Working with Argo_workflows..."

  @override
  def get_tool_processing_message(self) -> str:
    """Return message displayed when processing tool results."""
    return "📊 Processing argo_workflows data..."
