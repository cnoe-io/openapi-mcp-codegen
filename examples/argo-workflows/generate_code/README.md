# Argo_workflows Agent

This repository was auto-generated by *openapi-mcp-codegen*.

It wraps the **Argo_workflows MCP server** in a LangGraph
...
Follow the on-screen prompts to chat with your Argo_workflows tools!
...
|----------|----------|-------------|
| `ARGO_WORKFLOWS_API_URL` | Yes | URL of the backend service |
| `ARGO_WORKFLOWS_TOKEN` | Yes | Authentication token for the service |
| `ARGO_WORKFLOWS_VERIFY_SSL` | No | Enable/disable SSL certificate verification (default: `true`). Falls back to `VERIFY_SSL` if not set. |
| `ARGO_WORKFLOWS_CA_BUNDLE` | No | Path to custom CA bundle for SSL verification |
...
ARGO_WORKFLOWS_VERIFY_SSL=false make run-a2a

# Use custom CA bundle for SSL verification
ARGO_WORKFLOWS_CA_BUNDLE=/path/to/ca-bundle.pem make run-a2a

# Set via environment file
echo "A2A_PORT=9000" >> .env
echo "MCP_PORT=4000" >> .env
echo "ARGO_WORKFLOWS_VERIFY_SSL=false" >> .env
```
