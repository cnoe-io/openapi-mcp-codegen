...
verify_ssl_env = os.getenv("ARGO_WORKFLOWS_VERIFY_SSL", None)
if verify_ssl_env is None:
  verify_ssl_env = os.getenv("VERIFY_SSL", "true")
VERIFY_SSL = verify_ssl_env.lower() == "true"

CA_BUNDLE = os.getenv("ARGO_WORKFLOWS_CA_BUNDLE")

if not API_URL:
  raise ValueError("ARGO_WORKFLOWS_API_URL environment variable is not set.")
if not API_TOKEN:
  raise ValueError("ARGO_WORKFLOWS_TOKEN environment variable is not set.")

# Configure logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("mcp_argo_workflows")
...
async def make_api_request(
  ...
  Args:
      ...
      token: API token (defaults to ARGO_WORKFLOWS_TOKEN)
  ...
  try:
    headers_dict = {"Authorization": "Bearer {token}", "Accept": "application/json", "Content-Type": "application/json"}
