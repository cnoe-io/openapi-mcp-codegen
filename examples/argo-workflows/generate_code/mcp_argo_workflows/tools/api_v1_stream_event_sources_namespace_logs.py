...
async def get_evt_src_svc_evt_srcs_logs(
  path_namespace: str,
  param_name: str = None,
  param_event_source_type: str = None,
  param_event_name: str = None,
  param_grep: str = None,
  ...
  """
      ...
      Args:
          ...
          param_name (str): optional - only return entries for this event source.

          param_event_source_type (str): optional - only return entries for this event source type (e.g. `webhook`).

          param_event_name (str): optional - only return entries for this event name (e.g. `example`).

          param_grep (str): optional - only return entries where `msg` matches this regular expression.
  ...
  if param_event_source_type is not None:
    params["event_source_type"] = (
      str(param_event_source_type).lower() if isinstance(param_event_source_type, bool) else param_event_source_type
    )

  if param_event_name is not None:
    params["event_name"] = str(param_event_name).lower() if isinstance(param_event_name, bool) else param_event_name
