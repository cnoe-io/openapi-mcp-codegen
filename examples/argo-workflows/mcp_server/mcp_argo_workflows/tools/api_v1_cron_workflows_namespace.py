...
async def cron_workflow_service_list_cron_workflows(
  ...
  OpenAPI Description:
      Lists all cron workflows in a namespace. Use when: discovering scheduled workflows or monitoring cron job configurations.

  Args:

      path_namespace (str): "Kubernetes namespace to scope the cron workflow retrieval"
      ...
      param_listOptions_watch (bool): Stream real-time resource changes when true

      param_listOptions_allowWatchBookmarks (bool): Enable bookmark events in watch mode for progress tracking

      param_listOptions_resourceVersion (str): Specify resource version to filter results for consistency. Use for version-specific data.
      ...
      param_listOptions_limit (int): "Maximum cron workflows to return (default: 50)"
...
async def cron_workflow_service_create_cron_workflow(path_namespace: str) -> Any:
  """
  Create a new cron-workflows

  OpenAPI Description:
      Creates a new cron workflow in the specified namespace. Use when: scheduling recurring automated tasks or batch processes.

  Args:

      path_namespace (str): "Kubernetes namespace to scope the cron workflow operation"
