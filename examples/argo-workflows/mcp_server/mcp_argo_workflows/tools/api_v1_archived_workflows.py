...
async def archived_workflow_service_list_archived_workflows(
  ...
  param_namePrefix: str = None,
  param_namespace: str = None,
) -> Any:
  """
  List or query archived-workflows

  OpenAPI Description:
      Lists all archived workflows with optional filters. Use when: discovering past workflows or monitoring historical data.

  Args:

      param_listOptions_labelSelector (str): Criteria to filter workflows by labels (e.g., app=nginx)

      param_listOptions_fieldSelector (str): "Field criteria to filter results (e.g., status=completed)"
      ...
      param_listOptions_resourceVersion (str): "Specify resource version to filter archived workflows"
      ...
      param_listOptions_limit (int): "Maximum archived workflows to return (default: 50)"

      param_listOptions_continue (str): Pagination token for retrieving next page of results

      param_listOptions_sendInitialEvents (str): Send initial events in watch stream when true

      param_namePrefix (str): "Filter workflows starting with this prefix (e.g., 'test-')"
  ...
  if param_namePrefix is not None:
    params["namePrefix"] = str(param_namePrefix).lower() if isinstance(param_namePrefix, bool) else param_namePrefix
