...
async def workflow_service_watch_events(
  path_namespace: str,
  ...
  """
  Retrieve details of a specific events

  OpenAPI Description:
      Streams events in a namespace. Use when: monitoring real-time changes or tracking specific events in workflows.

  Args:

      path_namespace (str): "Kubernetes namespace to scope event retrieval"

      param_listOptions_labelSelector (str): Criteria to filter events by labels (e.g., app=nginx)
      ...
      param_listOptions_allowWatchBookmarks (bool): "Enable bookmark events in watch streams when true"

      param_listOptions_resourceVersion (str): Specify resource version to filter events from that point onward.

      param_listOptions_resourceVersionMatch (str): Controls resource version matching for list calls (e.g., exact, notOlderThan)

      param_listOptions_timeoutSeconds (int): Maximum time to wait for events in seconds (e.g., 30)

      param_listOptions_limit (int): "Maximum events to return (e.g., 100). Use to limit response size."

      param_listOptions_continue (str): Pagination token for retrieving next set of results
