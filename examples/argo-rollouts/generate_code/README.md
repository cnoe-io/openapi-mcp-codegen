...
It wraps the **Argo_rollouts MCP server** in a LangGraph
...
Follow the on-screen prompts to chat with your Argo_rollouts tools!
...
|----------|----------|-------------|
| `ARGO_ROLLOUTS_API_URL` | Yes | URL of the backend service |
| `ARGO_ROLLOUTS_TOKEN` | Yes | Authentication token for the service |
| `ARGO_ROLLOUTS_VERIFY_SSL` | No | Enable/disable SSL certificate verification (default: `true`). Falls back to `VERIFY_SSL` if not set. |
| `ARGO_ROLLOUTS_CA_BUNDLE` | No | Path to custom CA bundle for SSL verification |
...
ARGO_ROLLOUTS_VERIFY_SSL=false make run-a2a

# Use custom CA bundle for SSL verification
ARGO_ROLLOUTS_CA_BUNDLE=/path/to/ca-bundle.pem make run-a2a

# Set via environment file
echo "A2A_PORT=9000" >> .env
echo "MCP_PORT=4000" >> .env
echo "ARGO_ROLLOUTS_VERIFY_SSL=false" >> .env
```
