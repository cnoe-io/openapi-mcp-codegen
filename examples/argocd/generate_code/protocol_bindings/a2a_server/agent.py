# Copyright 2025 CNOE
"""Argocd LangGraph agent wrapper used by the A2A server.

This class inherits from BaseLangGraphAgent to leverage common A2A functionality
while providing argocd-specific configuration and MCP server bootstrap.
"""
...
class ArgocdAgent(BaseLangGraphAgent):
  """Argocd agent that inherits streaming and A2A protocol handling from BaseLangGraphAgent."""

  @override
  async def _bootstrap_agent(self) -> Any:
    """Bootstrap the argocd MCP server and return a LangGraph React agent."""
    load_dotenv()
    token = os.getenv("ARGOCD_TOKEN")
    api_url = os.getenv("ARGOCD_API_URL")

    if not token or not api_url:
      raise EnvironmentError("Both ARGOCD_API_URL and ARGOCD_TOKEN must be set")
  ...
  def _get_trace_tags(self) -> list[str]:
    """Return trace tags specific to argocd."""
    return ["argocd-a2a"]

  @override
  def _get_trace_name(self) -> str:
    """Return trace name for argocd queries."""
    return "argocd-query"
  ...
  def get_system_instruction(self) -> str:
    ...
    return """You are an expert ArgoCD assistant that helps users manage, deploy, and monitor applications using GitOps practices in Kubernetes.
