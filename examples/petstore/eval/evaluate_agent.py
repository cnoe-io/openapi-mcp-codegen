"""
Standalone evaluation script for the generated petstore agent.
...
try:
    ...
    from petstore.agent import create_agent, DEFAULT_SYSTEM_PROMPT
...
async def predict(inputs):
    ...
    try:
        lf = get_client()
        with lf.start_as_current_span(name="petstore-predict") as span:
...
async def main():
    ...
    lf = get_client()
    try:
        with lf.start_as_current_span(name="petstore-eval") as init_span:
            init_span.update_trace(tags=["petstore-eval"])
    except Exception:
        pass
    ...
    lf = get_client()
    dataset_name = "petstore_agent_eval"
    run_name = f"petstore_agent_eval_run_{uuid.uuid4().hex[:8]}"
    ...
    try:
        ...
        dataset = lf.create_dataset(
            name=dataset_name,
            description="Evaluation dataset for petstore agent",
        )
    ...
    async def _process_case(c: dict):
        ...
        if ds_item and hasattr(ds_item, "run"):
            run_ctx = ds_item.run(
                run_name=run_name,
                run_metadata={"agent": "petstore"},
                run_description=f"Evaluation run for petstore",
            )
        else:
            # Fallback: create a root span if linking fails
            run_ctx = lf.start_as_current_span(
                name=f"petstore-eval-item",
                input=item_input,
            )
