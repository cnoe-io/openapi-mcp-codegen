...
"""Interactive evaluation mode for the Petstore LangGraph agent.
...
def _prompt_for_query(
    ...
    def _suggest_query(cur_strategy: str) -> str:
        ...
        try:
            ...
            human_msg = HumanMessage(content=f"Tool: {tool_name}\nDescription: {tool_desc}\nStrategy: {cur_strategy}")
    ...
    while True:
        ...
        try:
            inp = _in("Enter a user query (press Enter to accept suggestion, (u)pdate strategy, (s)kip or (q)uit): ")
        except SkipTool:
            raise
...
async def _interactive_eval() -> None:
    ...
    try:
        strategy = _in("Optional global evaluation strategy (e.g. resource names) – press Enter, (s)kip or (q)uit: ")
    except SkipTool:
        strategy = ""
    except QuitEvaluation:
        print("Exiting evaluation session.")
    ...
    def _mark_skipped(tn: str) -> None:
        ...
        skip_idxs = [i for i, t in enumerate(dataset) if t.get("tool") == tn and len(t) == 1]
    ...
    try:
        get_client().update_current_trace(tags=["petstore-eval-mode"])
    except Exception:
        pass
    ...
    async def _rerun_all_existing(agent, dataset: list[dict], dest_path: Path) -> None:
        ...
        runnable = [(i, it) for i, it in enumerate(dataset) if {"tool", "input", "trajectory", "output"} <= set(it.keys())]
        if not runnable:
    ...
    try:
        while True:
            ...
            try:
                choice = _in(f"Select tool to evaluate [{default_tool}] (enter/number/(ra)=re-run all/(s)kip/(q)uit): ")
            except SkipTool:
            ...
            # ------------------------------------------------- dataset clash check
            existing_idx: list[int] = [i for i, t in enumerate(dataset) if t.get("tool") == tool_name]
            replace_existing = False
            if existing_idx:
                try:
                    choice = _in(
                        f"{len(existing_idx)} existing test(s) for '{tool_name}' found – (r)eplace, (a)dd as extra, (s)kip or (q)uit? [a]: "
                    ).lower()
            ...
            while True:
                ...
                try:
                    with langfuse.start_as_current_span(
                        name="petstore-eval-invoke",
                        input={"query": user_query, "tool": tool_name},
                    ) as span:
                        span.update_trace(tags=["petstore-eval"], session_id=cfg["configurable"]["thread_id"])
                        await agent.ainvoke(
                ...
                try:
                    choice = _in("Retry with different query (r), continue (enter), (s)kip or (q)uit: ").lower()
                except SkipTool:
                    rprint("[yellow]Skipped.[/]\n")
                    _mark_skipped(tool_name)
                    success = False
