...
async def find_pets_by_tags(param_tags: List[str] = None) -> Any:
  ...
  Multiple tags can be provided as a comma-separated list. For example: tag1, tag2, tag3.

  Args:
      param_tags (List[str], optional): List of tags to filter pets by. Defaults to None.

  Returns:
      Any: The JSON response from the API call containing pets matching the provided tags.
