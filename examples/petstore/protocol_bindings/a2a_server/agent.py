# Copyright 2025 CNOE
"""Petstore LangGraph agent wrapper used by the A2A server.
...
async def _bootstrap_langgraph_agent():
  ...
  if not token or not api_url:
    raise EnvironmentError("Both PETSTORE_API_URL and PETSTORE_TOKEN must be set")

  from agent import create_agent  # noqa: E402

  agent, _ = await create_agent()
  try:
    get_client().update_current_trace(tags=["petstore-a2a"])
  except Exception:
    pass
  return agent
  ...
  async def stream(
    ...
    async def _async_gen():
      with lf.start_as_current_span(
        ...
        root_span.update_trace(tags=["petstore-a2a"], session_id=session_id)
